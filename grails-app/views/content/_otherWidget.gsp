...
<table>
	<tbody>
		<tr style="display: block;">
			<td>
				<div class="topic_addition_widget" style="width: 728px;min-height: 655px;">
					<div class="form_title removeBorder">
						<span style="margin-left: 5px;">
							<button id="topicSchedule" class="showHover" title="${Help.toolTip("CONTENT", "Topic Schedule")}" onclick="showTopicSchedule()">
								Topic Schedule
							</button>
						</span>
					</div>
	...
	<g:form controller="content" action="saveTopicSchedule" name="topicScheduleForm">
		...
		<table>
			<tbody>
				<tr>
					<td>
						<input type="button" name="addTopicScheduleButton" onclick="addTopicScheduleAfter()" class="add" value="Add Topic Schedule" style="padding: 0;width:200px;"/>
					</td>
					<td>
						<input type="button" name="removeTopicScheduleButton" onclick="removeTopicScheduleBefore()" style="padding: 0;width:250px;" class="remove" value="Remove Topic Schedule"/>
					</td>
					<td>
						<input type="button" name="saveTopicScheduleButton" onclick="jQuery('form#topicScheduleForm').submit()" class="add" value="Save" style="padding: 0;width:88px;">
					</td>
				</tr>
			</tbody>
		</table>
		<table id="topicScheduleMainTable"  style="width: 400px;">
			...
			<g:if test="${topicDateForCurrentLearningObjectiveList}">
				<g:each in="${topicDateForCurrentLearningObjectiveList}" var="topicDateForCurrentLearningObjective">
					<tr style="border:1px solid #000" class='contentTopicSchedule' id="${topicDateForCurrentLearningObjective?.id}" dir="${topicDateForCurrentLearningObjective?.id}">
						...
						<td>
							<g:datePicker name="scheduleDate" class="scheduleDate" precision="day" value="${topicDateForCurrentLearningObjective?.startDate}"/>
						</td>
					</tr>
				</g:each>
			</g:if>
			...
			<tr>
				<td colspan="2">
					...
					<input type="button" value="Cancel" name="topicScheduleCancel" style="padding: 0" onclick="closeTopicScheduleTable()"/>
				</td>
			</tr>
		</table>
	</g:form>
...
<div id="templateForTopicSchedule" style="display: none;">
	<table border="1">
		<tr class='contentResources' dir="0" style="border: 1px solid #000;">
			...
			<td>
				<g:datePicker name="scheduleDate" class="scheduleDate" precision="day" value=""/>
			</td>
		</tr>
	</table>
</div>
