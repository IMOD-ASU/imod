...
<html>
	...
	<body>
		<a href="#list-actionWord" class="skip" tabindex="-1">
				...
				<tbody>
					<g:each in="${actionWordInstanceList}" status="i" var="actionWordInstance">
						<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							<td>
								<g:link action="show" id="${actionWordInstance.id}">${fieldValue(bean: actionWordInstance, field: "actionWord")}</g:link>
							</td>
							<td>
								${fieldValue(bean: actionWordInstance, field: "category")}
							</td>
							<td>
								${fieldValue(bean: actionWordInstance, field: "domain")}
							</td>
							<td>
								${fieldValue(bean: actionWordInstance, field: "user")}
							</td>
							<td>
								${fieldValue(bean: actionWordInstance, field: "visibility")}
							</td>
						</tr>
					</g:each>
				</tbody>
