...
<html>
	...
	<body>
		<a href="#list-contentSchedule" class="skip" tabindex="-1">
				...
				<tbody>
					<g:each in="${contentScheduleInstanceList}" status="i" var="contentScheduleInstance">
						<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							<td>
								<g:link action="show" id="${contentScheduleInstance.id}">
									${fieldValue(bean: contentScheduleInstance, field: "content")}
								</g:link>
							</td>
							<td>
								${fieldValue(bean: contentScheduleInstance, field: "imod")}
							</td>
							<td>
