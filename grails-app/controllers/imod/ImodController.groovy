...
class ImodController {
	...
	def list() {
		...
		// generate sample imod if it doesn't already exist
		generateSample(currentUser)

		// search for imods owned by current user
		final displayList = Imod.findAllWhere(owner: currentUser)
	...
	private generateSample (currentUser) {
		...
		if (sample == null) {
			...
			final interpret = ActionWordCategory.findByActionWordCategory('Interpret')
			final implement = ActionWordCategory.findByActionWordCategory('Implement')
			final audience1 = Audience.findByDescription('Lower Division')
