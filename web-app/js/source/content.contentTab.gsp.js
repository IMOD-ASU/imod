jQuery(document).ready(function () {
	...
	jQuery("#showTopicScheduleTable").dialog({ autoOpen: false, height: 350, width: 600, show: "fadein", hide: "fadeout", modal: true });
	...
	if (id) {
		if (confirm("Are you sure?")) {
			jQuery.ajax({
				...
				success: function (data) {
					if (data == "success") {
						uery('tr#' + contentResourceIdElement).remove();
					}
				}
			});
		}
